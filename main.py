"""
Main script: orchestrazione pipeline
1) se manca il .npz, lo genera con preprocess_data.py
2) poi train + plot
"""
import os
import torch
...
# 1) genera .npz se non esiste
if not os.path.exists("preprocessed_bearing_data.npz"):
    print("⚙️  preprocessed_bearing_data.npz non trovato → preprocessing")
...
def main():
    ...
    # 7) training loop (50 epoche)
    for epoch in range(1,51):
        lambda_coef = 0.001 * min(1.0, epoch/10.0)
        train_epoch(
            model, train_loader, optimizer, device,
            lambda_coef, {"risk_weight":1.0, "target_uncertainty":0.06}
        )
