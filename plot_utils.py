...
def plot_training_curves(train_hist, test_hist):
    ...
    ax.set_xlabel("Epoch"); ax.set_ylabel("Total evidence"); ax.legend()
    # (b) training acc & unc
    ax = axes[0,1]
    ax.plot(epochs, train_hist["accuracy"], 'b-', label="acc")
    ...
    ax.set_xlabel("Epoch"); ax.set_ylabel("Uncertainty"); ax.legend()
    plt.tight_layout()
...
def plot_sample_bar(model, X_full, device, num_classes):
    ...
    for c in range(num_classes):
        ax.bar(x + (c - num_classes/2)*w, P[:,c], width=w, alpha=0.7)
    ax2 = ax.twinx()
