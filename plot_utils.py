...
def plot_training_curves(train_hist, test_hist):
    ...
    ax.set_xlabel("Epoch"); ax.set_ylabel("Estimated total evidence")
    ax.legend()
    ...
    # (d)
    ax=axes[1,1]
    ax.plot(epochs,test_hist['corr_unc'],'r--')
    ax.plot(epochs,test_hist['mis_unc'],'k--')
    ax.set_title("(d) Testing uncertainty")
    ax.set_xlabel("Epoch"); ax.set_ylabel("Uncertainty")
    ax.legend(["unc corr","unc mis"])

    plt.tight_layout()
    plt.savefig("article_style_evi_unc.png")
    plt.show()
